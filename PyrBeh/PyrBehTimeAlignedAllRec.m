function PyrBehTimeAlignedAllRec(pathS,onlyRun,task)
%% accumulating information about animals' behavior over time across recordings
%% Data aligned to run onset
    ...
    if(task == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalBehTime\'];
    elseif(task == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalBehTimeALPL\'];
    else
        pathAnal = [pathS '\PyramidalBehTimeAL\'];
    end
...
%     end
    ...
    behAlignedMeanFieldStat = accumMeanBehTimeStat(behAlignedMeanField,behAlignedAL.spaceStepsLick,...
        behAlignedAL.spaceStepsSpeed);
...
%     plotSpeed(pathAnal,behAlignedAL.spaceStepsSpeed,...
