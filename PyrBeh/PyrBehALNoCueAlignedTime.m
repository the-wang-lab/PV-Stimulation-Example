function PyrBehALNoCueAlignedTime(pathS,onlyRun)
...
%% plot aligned to run and over time (onlyRun should be set to 0)

    RecordingListPyrInt;  % include all the early NoCue (no blackout), PL and AL (up to A057)    
    pathAnal = [pathS '\PyramidalBehALNoCueAlignedTime\'];
    if(exist(pathAnal) == 0)
        mkdir(pathAnal);
    end
