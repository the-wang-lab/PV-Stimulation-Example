function PyrInitPeakAllRec(pathS,taskSel,methodKMean)
%% accumulating the information about the average firing rate profile for individual pyramidal neurons across recordings
%% data aligned to run onset
%% non stim trials, separating into good and bad trials
...
%     if(taskSel == 2 && exist('modPyr1NoCue') == 0)
        ...
        if(~isempty(numField.NoCue))
            ...
            numField.pRSNumField = 0;
            numField.pRSPercField = 0;
        end
        ...
        if(~isempty(numField.NoCue))
            ...
            numField.pRSNumFieldAL = 0;
            numField.pRSPercFieldAL = 0;
        end
        ...
        if(~isempty(numFieldAligned.NoCue))
            ...
            numFieldAligned.pRSNumField = 0;
            numFieldAligned.pRSPercField = 0;
        end
        ...
        if(~isempty(numFieldAligned.NoCue))
            ...
            numFieldAligned.pRSNumFieldAL = 0;
            numFieldAligned.pRSPercFieldAL = 0;
        end
...
%     end
    ...
    %% accumulate across different types of tasks
    ...
    FRProfileMean = accumMean(init.avgFRProfile,modPyr1AL.timeStepRun);
    
    FRProfileMeanGood = accumMean(init.avgFRProfileGood,modPyr1AL.timeStepRun); % good trials only
    
    FRProfileMeanBad = accumMean(init.avgFRProfileBad,modPyr1AL.timeStepRun); % bad trials only
...
%     plotPyrInitPeakClu(pathAnal,init.idxCGood,FRProfileMeanGood,FRProfileMeanStatCGood,colorSel,idxCTmp,'Good');
    ...
    %% fields two clusters
%     plotPyrInitFieldRecPerClu(pathAnal,init.idxC,init.nNeuWithField,modPyr1AL.timeStepRun,...
...
%         modPyr1AL.timeStepRun,init.avgFRProfile,init.avgFRProfileNorm,...
...
%     pause;
    ...
    %% fields all cells
    plotPyrInitFieldRec(pathAnal,init.nNeuWithField,modPyr1AL.timeStepRun,...
...
%     plotPyrInitFieldRecPerClu(pathAnal,init.idxC,init.nNeuWithFieldAligned,modPyr1AL.timeStepRun,...
...
%         FRProfileMean,FRProfileMeanStatCAligned,colorSel,idxCTmp,'Aligned');
    ...
    plotPyrInitFieldRec(pathAnal,init.nNeuWithFieldAligned,modPyr1AL.timeStepRun,...
...
%     pause;
    ...
    plotPyrInitGoodBadTrPerClu(pathAnal,init.idxCGood,init.idxCBad,modPyr1AL.timeStepRun,...
...
%     plotPyrInitCmpClu(pathAnal,idxC1,idxC2,modPyr1AL.timeStepRun,init.avgFRProfile,...
%         init.avgFRProfileNorm,FRProfileMean,FRProfileMeanStatC1C2,colorSel);
%     
%     plotPyrInitGoodVsBadTr(pathAnal,modPyr1AL.timeStepRun,init.avgFRProfileGood,init.avgFRProfileBad,...
