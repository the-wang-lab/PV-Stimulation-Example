function PyrModAlignedAllRec(pathS,onlyRun,taskSel,methodKMean)
...
%% Data are aligned to run onset 
    ...
    pathAnal1 = [pathS '\Pyramidal\'];
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalAligned\' num2str(methodKMean) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalAlignedALPL\' num2str(methodKMean) '\'];
    else % AL neurons only
        pathAnal = [pathS '\PyramidalAlignedAL\' num2str(methodKMean) '\'];
    end
