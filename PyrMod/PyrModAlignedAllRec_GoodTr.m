function PyrModAlignedAllRec_GoodTr(pathS,onlyRun,taskSel,methodKMean)
...
%% separating good and bad trials
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalAlignedGoodTr\' num2str(methodKMean) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalAlignedGoodTrALPL\' num2str(methodKMean) '\'];
    else % AL neurons only
        pathAnal = [pathS '\PyramidalAlignedGoodTrAL\' num2str(methodKMean) '\'];
    end
