function PyrModAlignedDistAllRec_GoodTr(pathS,onlyRun,taskSel,methodKMean)
%% accumulate information about individual pyramidal neurons' 
%% activity corr in distance across recordings
%% Data are aligned to run onset 
%% separating good vs. bad trials
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalAlignedDistGoodTr\' num2str(methodKMean) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalAlignedDistGoodTrALPL\' num2str(methodKMean) '\'];
    else % AL neurons only
        pathAnal = [pathS '\PyramidalAlignedDistGoodTrAL\' num2str(methodKMean) '\'];
    end
