function PyrModAllRec(pathS,onlyRun,taskSel,methodKMean)
%% accumulate data from different task types
%% perform statistical tests on different neuronal clusters derived from function PyrPropAllRec
%% e.g. PyrModAllRec(''Z:\Yingxue\Draft\PV\',1,1,2)
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\Pyramidal\' num2str(methodKMean) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalALPL\' num2str(methodKMean) '\'];
    else % AL neurons only
        pathAnal = [pathS '\PyramidalAL\' num2str(methodKMean) '\'];
    end
