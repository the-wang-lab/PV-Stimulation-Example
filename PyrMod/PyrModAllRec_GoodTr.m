function PyrModAllRec_GoodTr(pathS,onlyRun,taskSel,methodKMean)
...
%% separating good and bad trials.
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalGoodTr\' num2str(methodKMean) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalGoodTrALPL\' num2str(methodKMean) '\'];
    else % AL neurons only
        pathAnal = [pathS '\PyramidalGoodTrAL\' num2str(methodKMean) '\'];
    end
