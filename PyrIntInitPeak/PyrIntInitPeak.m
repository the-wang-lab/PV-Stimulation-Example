function PyrIntInitPeak(pathS,taskSel)
%% compare Pyramidal neurons and interneurons on their initial peak
%% Separating pyramidal neurons into PyrRise, PyrDown and PyrOthers based on their FRaft/FRbef
    
    pathAnal0 = [pathS '\Pyramidal\'];
    pathAnalInt0 = [pathS '\Interneuron\'];
    ...
    pathAnalPeak0 = [pathS '\PyramidalInitPeakAL\2\'];
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalIntInitPeak\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalIntInitPeakALPL\'];
    else
        pathAnal = [pathS '\PyramidalIntInitPeakAL\'];
    end
    if(~exist(pathAnal))
        mkdir(pathAnal);
    end
    ...
    %     idx1 = find(ratio0to1BefRunOrd >= 0.5,1,'last');
        ...
        FRProfileMeanPyr.OtherField = accumMeanPVStim(InitAll.avgFRProfile(isNeuWithFieldAligned.idxOther,:),...
            modPyr1AL.timeStepRun); % added on 7/8/2022
        ...
        FRProfileMeanPyr.Rise = accumMeanPVStim(InitAll.avgFRProfile(PyrRise.idxRise,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.Down = accumMeanPVStim(InitAll.avgFRProfile(PyrDown.idxDown,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.Other = accumMeanPVStim(InitAll.avgFRProfile(PyrOther.idxOther,:),modPyr1AL.timeStepRun); % added on 7/8/2022

        FRProfileMeanPyr.RiseBad = accumMeanPVStim(InitAll.avgFRProfileBad(PyrRise.idxRiseBadBad,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.DownBad = accumMeanPVStim(InitAll.avgFRProfileBad(PyrDown.idxDownBadBad,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.OtherBad = accumMeanPVStim(InitAll.avgFRProfileBad(PyrOther.idxOtherBadBad,:),modPyr1AL.timeStepRun); % added on 7/8/2022
...
%     time50percAvgFRPyrRise = findHalfWayRiseTime(InitAll.avgFRProfile(PyrRise.idxRise,:),modPyr1AL.timeStepRun,1,0);
%     
%     % PyrDown neurons 50% ramping time
%     time50percAvgFRPyrDown = findHalfWayRiseTime(InitAll.avgFRProfile(PyrDown.idxDown,:),modPyr1AL.timeStepRun,2,0);
%     
%     %% for all the interneurons
...
%     for i = 1:max(InitAll.idxC2Int)
%         time50percAvgFRIntRise{i} = findHalfWayRiseTime(InitAll.avgFRProfileInt(IntRise.idxRise{i},:),modPyr1AL.timeStepRun,1,0);
...
%     %% plot depth of rise and down pyramidal neurons
    ...
    plotPyrNeuRiseDownField(pathAnal,modPyr1AL.timeStepRun,InitAll,...
            isNeuWithFieldAligned,FRProfileMean,FRProfileMeanPyr,FRProfileMeanPyrStat,colorSel,[{'Good'} {'Bad'}]);
    close all;
    
    plotPyrNeuRiseDown(pathAnal,modPyr1AL.timeStepRun,InitAll,PyrRise,PyrDown,PyrOther,...
