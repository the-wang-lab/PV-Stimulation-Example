function PyrIntInitPeakSig(pathS,taskSel,pshuffle)
%% compare Pyramidal neurons and interneurons on their initial peak
%% Separating pyramidal neurons into PyrRise, PyrDown and PyrOthers based on the significance of their FRaft/FRbef
%% (pshuffle = 1 -> 99.9%, 2 -> 99%, 3 -> 95%)
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = [pathS '\PyramidalIntInitPeakSig\' num2str(pshuffle) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = [pathS '\PyramidalIntInitPeakSigALPL\' num2str(pshuffle) '\'];
    else
        pathAnal = [pathS '\PyramidalIntInitPeakSigAL\' num2str(pshuffle) '\'];
    end
    if(exist(pathAnal) == 0)
        mkdir(pathAnal);
    end
    ...
    if(exist('InitAllPyr') == 0)
        ...
        FRProfileMeanPyr.Rise = accumMeanPVStim(InitAllPyr.avgFRProfile(PyrRise.idxRise,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.Down = accumMeanPVStim(InitAllPyr.avgFRProfile(PyrDown.idxDown,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.Other = accumMeanPVStim(InitAllPyr.avgFRProfile(PyrOther.idxOther,:),modPyr1AL.timeStepRun); % added on 7/8/2022

        FRProfileMeanPyr.RiseBad = accumMeanPVStim(InitAllPyr.avgFRProfileBad(PyrRise.idxRiseBadBad,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.DownBad = accumMeanPVStim(InitAllPyr.avgFRProfileBad(PyrDown.idxDownBadBad,:),modPyr1AL.timeStepRun);
        FRProfileMeanPyr.OtherBad = accumMeanPVStim(InitAllPyr.avgFRProfileBad(PyrOther.idxOtherBadBad,:),modPyr1AL.timeStepRun); % added on 7/8/2022
...
%     close all;
    ...
    plotPyrNeuRiseDownSig(pathAnal,modInt1AL.timeStepRun,InitAllPyr,PyrRise,PyrDown,PyrOther,...
