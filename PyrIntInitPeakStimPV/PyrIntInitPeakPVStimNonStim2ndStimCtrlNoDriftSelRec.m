function PyrIntInitPeakPVStimNonStim2ndStimCtrlNoDriftSelRec(pathS,methodKMean,pdrift)
%% Separating into rise and down neurons 
%% selecting neurons to remove the recordings with drifted firing rate between ctrl and stimCtrl. 
%% Excluding some recordings that have clear drifts during the PV activation experiments 
    
    pathAnal0 = [pathS '\PyramidalStimALNonStim2ndStimCtrl-NoDriftSelRec\'];
    pathAnal = [pathS '\PyramidalStimALNonStim2ndStimCtrlSig\' num2str(methodKMean) '\'];
...
%     end
    ...
    for i = 1:size(avgFRProfileStimCtrl,1)
     ...
     save([pathAnal0 'initPeakPyrIntAllRecStimSigNoStim2ndStimCtrl_avgProfile.mat'],'PyrStim','-v7.3');
        ...
        for i = 1
            ...
            for j = 1:length(pulseMethod{i})
                ...
                for m = 1:length(stimLocs)  
                    ...
                    %% added on 7/8/2022
                    if(~isempty(PyrOther.FRProfileMeanField{cond}.meanAvgFRProfileAll))
                        % compare good non-stim and stim trials other neurons
                        ...
                        PyrOther.FRProfileMeanStatNonStimVsStimField{cond} = [];
                        PyrOther.FRProfileMeanStatStimVsStimCtrlField{cond} = [];
                    end
