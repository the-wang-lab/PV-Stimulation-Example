...
%% accumulating information about the firing rate of each neuron, 
%% separating neurons into different groups depending on the opto-stimulation protocol
%% data aligned to run onset

%% This function is called by PyrInitPeakAllStimPVRecSigNonStim2ndStimCtrlTr and PyrInitPeakAllStimPVRecSigNonStim2ndStimCtrlTrGoodOnly
        ...
        for j = 1:length(pulseMethod{i})
            ...
            for m = 1:length(stimLocs)  
                ...
                if(~isempty(ind))
                    ...
                    FRProfileMeanStim{cond} = [];

                    FRProfileMeanStimCtrl{cond} = [];

                    % compare good non-stim and stim trials
                    FRProfileMeanStatGoodNonStimVsStim{cond} = [];

                    % compare stim ctrl and stim trials
                    FRProfileMeanStatStimCtrlVsStim{cond} = [];
                end
            end
        end
    end
